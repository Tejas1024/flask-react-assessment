...
class TestCommentApi(BaseTestComment):
    ...
    def test_create_comment_no_auth(self):
        account, _, task = self.setup_account_and_task()
        
        with self.app.test_client() as client:
            response = client.post(
                f"http://127.0.0.1:8080/api/accounts/{account.id}/tasks/{task.id}/comments",
                headers=self.HEADERS,
                data='{"content": "Test"}'
            )
        
        assert response.status_code == 401
        assert response.json['code'] == AccessTokenErrorCode.AUTHORIZATION_HEADER_NOT_FOUND
